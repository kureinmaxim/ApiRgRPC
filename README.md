...
- 🎛 **Device-control в GUI и движке**: панель HA‑моста (ping/read/write/stream)
  поверх RNS — общий код `device_control.py` для CLI и sidecar'а.
- 🔌 **Экран интерфейсов**: TCP (к мосту/VPS) и/или I2P из GUI — пишет
  `<app data>/reticulum/config`, применяется перезапуском движка.
...
Полная инструкция (macOS и Windows, бандлинг движка, подпись, грабли) —
**[BUILD.md](BUILD.md)**. Кратко:
